...
namespace nlrender
{
  ...
  public:
    ...
    NLRENDER_API
    void render(
      const unsigned int& texture_,
      const unsigned int& textureSize_,
      const Eigen::Matrix4f& modelMatrix_ = Eigen::Matrix4f::Identity( ),
      const Eigen::Vector3f& color_ =
      Eigen::Vector3f( 0.2f, 0.2f, 0.2f )) const;
    ...
    NLRENDER_API
    void render(
      const std::vector< unsigned int >& textures_,
      const unsigned int& textureSize_,
      const std::vector< Eigen::Matrix4f >& modelMatrices_,
      const Eigen::Vector3f& color_ =
      Eigen::Vector3f( 0.2f, 0.2f, 0.2f )) const;
    ...
    nlgeometry::MeshPtr extract(
      const unsigned int& texture_,
      const unsigned int& textureSize_,
      const Eigen::Matrix4f& modelMatrix_ = Eigen::Matrix4f::Identity( )) const;
